...
if use_gpu:
    ...
    model_conv.load_state_dict(torch.load(resume_file, map_location={'cuda:0':'cpu'}))
model_conv.eval()
...
for i, (XI, ims) in enumerate(trainloader):
    ...
    # 确定车牌颜色
    cropImage = img[int(left_up[1]) : int(right_down[1]), int(left_up[0]) : int(right_down[0])]
    green = yello = blue = black = white = 0
    img_hsv = cv2.cvtColor(cropImage, cv2.COLOR_BGR2HSV)
    ...
    limit1 = limit2 = 0
    if yello * 2 >= img_count:
        color = "黄色"
        limit1 = 11
        limit2 = 34    # 有的图片有色偏偏绿
    elif green * 2 >= img_count:
        color = "绿色"
        limit1 = 35
        limit2 = 99
    elif blue * 2 >= img_count:
        color = "蓝色"
        limit1 = 100
        limit2 = 124   # 有的图片有色偏偏紫
    elif black + white >= img_count * 0.7: #TODO
        color = "黑白"
    ...
    lpn = provinces[labelPred[0]] + alphabets[labelPred[1]] + \
        ads[labelPred[2]] + ads[labelPred[3]] + ads[labelPred[4]] + ads[labelPred[5]] + ads[labelPred[6]] + "   " + color
    print(ims[0].split('/')[-1] + ":" + lpn)
    font = ImageFont.truetype('../fonts/FangSong.otf', 40, encoding="utf-8")
    draw.text((int(left_up[0]), int(left_up[1]) - 40), lpn, (255, 0, 0), font=font)
    cv2charimg = cv2.cvtColor(np.array(pilimg), cv2.COLOR_RGB2BGR)
    # cv2.putText(img, lpn, (int(left_up[0]), int(left_up[1])-20), cv2.FONT_ITALIC, 2, (0, 0, 255))
    cv2.imwrite('../tests/res/' + ims[0].split('/')[-1], cv2charimg)
